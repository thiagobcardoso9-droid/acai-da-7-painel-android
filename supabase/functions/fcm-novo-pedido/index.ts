...
async function getGoogleAccessToken(serviceAccount: Record<string, string>): Promise<string> {
  ...
  const claim = base64UrlText(JSON.stringify({ iss: serviceAccount.client_email, scope: 'https://www.googleapis.com/auth/firebase.messaging', aud: 'https://oauth2.googleapis.com/token', iat: now, exp: now + 3600 }));
...
function orderText(record: Record<string, unknown>): { title: string; body: string; total: string } {
  const number = record.numero_dia ?? record.numero ?? record.id ?? '';
  const total = Number(record.total ?? record.valor_total ?? 0);
  const totalText = total > 0 ? total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' }) : '';
  return { title: '🍧 Açaí da 7 — NOVO PEDIDO!', body: `Pedido #${number}${totalText ? ` • ${totalText}` : ''} recebido. Toque para abrir o painel.`, total: totalText };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!expectedSecret || req.headers.get('x-webhook-secret') !== expectedSecret) return new Response(JSON.stringify({ error: 'unauthorized' }), { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    const payload = await req.json();
    if (payload?.table && payload.table !== 'pedidos') return new Response(JSON.stringify({ ok: true, ignored: true }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    ...
    // DATA-ONLY + HIGH: deixa o FirebaseMessagingService do APK montar a notificação
    // mesmo quando o aplicativo está em segundo plano/fechado e aplicar o toque escolhido.
    ...
    return new Response(JSON.stringify({ ok: true, fcm: JSON.parse(result) }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  } catch (error) {
    console.error(error);
