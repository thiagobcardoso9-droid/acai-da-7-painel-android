...
public class MainActivity extends ComponentActivity {
    ...
    private void requestNotificationPermission() {
        notificationPermissionLauncher = registerForActivityResult(new ActivityResultContracts.RequestPermission(), granted -> { if (granted) subscribeToNewOrders(); });
    ...
    @Override protected void onResume() {
        super.onResume();
        if (Build.VERSION.SDK_INT < 33 || ContextCompat.checkSelfPermission(this, Manifest.permission.POST_NOTIFICATIONS) == PackageManager.PERMISSION_GRANTED) subscribeToNewOrders();
    }

    private void configureWebView(WebView wv) {
        WebSettings s = wv.getSettings();
        s.setJavaScriptEnabled(true); s.setDomStorageEnabled(true); s.setDatabaseEnabled(true); s.setAllowFileAccess(true); s.setAllowContentAccess(true); s.setMediaPlaybackRequiresUserGesture(false); s.setSupportZoom(false);
    ...
    private boolean handleUrl(String url) { if(url==null)return false; Uri uri=Uri.parse(url); String host=uri.getHost(); if(host!=null&&host.equals("acaida7-painel.netlify.app"))return false; if(url.startsWith("file://"))return false; return openExternal(url); }
    private boolean openExternal(String url) { try{startActivity(new Intent(Intent.ACTION_VIEW,Uri.parse(url)));return true;}catch(Exception e){Toast.makeText(this,"Não foi possível abrir o link",Toast.LENGTH_SHORT).show();return true;} }
    ...
    public void setNotificationSound(int number){int n=Math.max(1,Math.min(50,number));int old=getNotificationSound();getSharedPreferences(PREFS_NATIVE,MODE_PRIVATE).edit().putInt(PREF_SOUND,n).apply();if(Build.VERSION.SDK_INT>=Build.VERSION_CODES.O){NotificationManager nm=getSystemService(NotificationManager.class);if(old!=n){try{nm.deleteNotificationChannel(channelId(old));}catch(Exception ignored){}}createNotificationChannel(n);}}
    ...
    private void createNotificationChannel(int soundNumber){
        if(Build.VERSION.SDK_INT<Build.VERSION_CODES.O)return;NotificationManager nm=getSystemService(NotificationManager.class);String id=channelId(soundNumber);if(nm.getNotificationChannel(id)!=null)return;int resId=soundResId(soundNumber);Uri soundUri=resId!=0?Uri.parse("android.resource://"+getPackageName()+"/"+resId):Uri.parse("android.resource://"+getPackageName()+"/"+R.raw.novo_pedido);NotificationChannel channel=new NotificationChannel(id,"Novos pedidos — toque "+String.format(java.util.Locale.US,"%02d",soundNumber),NotificationManager.IMPORTANCE_HIGH);channel.setDescription("Alertas de novos pedidos do Açaí da 7");channel.enableVibration(true);channel.setVibrationPattern(new long[]{0,300,150,500,150,700});channel.setSound(soundUri,new AudioAttributes.Builder().setUsage(AudioAttributes.USAGE_NOTIFICATION).setContentType(AudioAttributes.CONTENT_TYPE_SONIFICATION).build());nm.createNotificationChannel(channel);
    }
    public void previewNotificationSound(int number){int resId=soundResId(number);if(resId==0){Toast.makeText(this,"Toque não encontrado",Toast.LENGTH_SHORT).show();return;}try{MediaPlayer player=MediaPlayer.create(this,resId);if(player==null)return;player.setOnCompletionListener(MediaPlayer::release);player.setOnErrorListener((mp,what,extra)->{mp.release();return true;});player.start();}catch(Exception e){Toast.makeText(this,"Não foi possível testar o toque",Toast.LENGTH_SHORT).show();}}
    public void openNotificationSettings(){try{Intent intent=new Intent(Settings.ACTION_APP_NOTIFICATION_SETTINGS);intent.putExtra(Settings.EXTRA_APP_PACKAGE,getPackageName());startActivity(intent);}catch(Exception e){startActivity(new Intent(Settings.ACTION_SETTINGS));}}

    public void printHtml(String html){runOnUiThread(()->{try{if(printWebView!=null){printWebView.stopLoading();printWebView.destroy();printWebView=null;}printWebView=new WebView(this);WebSettings settings=printWebView.getSettings();settings.setJavaScriptEnabled(false);settings.setDefaultTextEncodingName("UTF-8");printWebView.setWebViewClient(new WebViewClient(){@Override public void onPageFinished(WebView view,String url){super.onPageFinished(view,url);startPrintJob();}});printWebView.loadDataWithBaseURL(PANEL_URL,html,"text/html","UTF-8",null);}catch(Exception e){Toast.makeText(this,"Não foi possível preparar a comanda para impressão.",Toast.LENGTH_LONG).show();}});}
    private void startPrintJob(){if(printWebView==null)return;try{PrintManager printManager=(PrintManager)getSystemService(Context.PRINT_SERVICE);PrintDocumentAdapter adapter=printWebView.createPrintDocumentAdapter("AcaiDa7-Comanda");PrintAttributes.MediaSize receipt58=new PrintAttributes.MediaSize("ACAI58","Comanda 58mm",2283,10000);PrintAttributes attributes=new PrintAttributes.Builder().setMediaSize(receipt58).setMinMargins(PrintAttributes.Margins.NO_MARGINS).build();printManager.print("Açaí da 7 — Comanda",adapter,attributes);}catch(Exception e){Toast.makeText(this,"Não foi possível abrir a impressão do Android.",Toast.LENGTH_LONG).show();}}

    private void checkForNativeUpdate(){new Thread(()->{HttpURLConnection connection=null;try{URL url=new URL(UPDATE_API_URL);connection=(HttpURLConnection)url.openConnection();connection.setRequestMethod("GET");connection.setConnectTimeout(8000);connection.setReadTimeout(8000);connection.setRequestProperty("Accept","application/vnd.github+json");connection.setRequestProperty("User-Agent","AcaiDa7-Painel-Android");if(connection.getResponseCode()!=HttpURLConnection.HTTP_OK)return;BufferedReader reader=new BufferedReader(new InputStreamReader(connection.getInputStream()));StringBuilder response=new StringBuilder();String line;while((line=reader.readLine())!=null)response.append(line);reader.close();JSONObject release=new JSONObject(response.toString());String body=release.optString("body","");Matcher versionMatcher=Pattern.compile("VERSION_NAME=([^\\s\\n]+)").matcher(body);if(!versionMatcher.find())return;String latestVersion=versionMatcher.group(1).trim();String currentVersion=getPackageManager().getPackageInfo(getPackageName(),0).versionName;if(!isNewerVersion(latestVersion,currentVersion))return;JSONArray assets=release.optJSONArray("assets");if(assets==null)return;String apkUrl=null;for(int i=0;i<assets.length();i++){JSONObject asset=assets.getJSONObject(i);if(UPDATE_ASSET_NAME.equals(asset.optString("name"))){apkUrl=asset.optString("browser_download_url",null);break;}}if(apkUrl==null||apkUrl.isEmpty())return;final String downloadUrl=apkUrl;runOnUiThread(()->showUpdateDialog(latestVersion,downloadUrl));}catch(Exception ignored){}finally{if(connection!=null)connection.disconnect();}}).start();}
    private boolean isNewerVersion(String latest,String current){try{String[] a=latest.replaceAll("[^0-9.]","").split("\\.");String[] b=current.replaceAll("[^0-9.]","").split("\\.");int max=Math.max(a.length,b.length);for(int i=0;i<max;i++){int av=i<a.length&&!a[i].isEmpty()?Integer.parseInt(a[i]):0;int bv=i<b.length&&!b[i].isEmpty()?Integer.parseInt(b[i]):0;if(av!=bv)return av>bv;}}catch(Exception ignored){}return false;}
